...
- 2026-09-22: GitHub-интеграция (вход по PAT — OAuth для локального тула барьер, токен в ~/.project-butler/github.token, в remote не пишется): публикация (создание repo + push с одноразовым токен-URL), приватность/описание/issues/wiki (PATCH), релизы; настройки UI — 4 темы + свой стиль (4 цвета через /api/settings), i18n RU/EN всей поверхности; канвас-фон красится палитрой темы. 45 тестов + смоук.
- 2026-09-22 (ночь 2): сверх плана — Wrapped (store.wrapped + /api/wrapped + панель «отчёт миссии»: дни в полёте, лучший/худший/заброшенный, risen/fallen, TODO-динамика), TODO-список в карточке с переходом vscode://file/...:line, Git-пульс (30 дней столбиками, /api/gitpulse), watch-сервер (поллинг mtime корней раз в 90с, авто-рескан через ScanState, тосты Windows через butler/notify.py), npm audit + pip-audit в докторе, мульти-корень (settings.multi, build_feed_multi, скан/дайджест/wrapped по всем отмеченным корням, дубликаты ищутся между корнями). 45 тестов + смоук.
